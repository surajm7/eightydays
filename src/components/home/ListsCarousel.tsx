...
export default function ListsCarousel({ lists, title = "Try Smart Search" }: ListsCarouselProps) {
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (scrollContainerRef.current) {
      const cardWidth = 320 // Increased for better layout
  ...
  // Calculate price based on list stats (for demo purposes)
  const calculatePrice = (list: List) => {
    const basePrice = 1200 + (list.stats.place_count * 150)
    ...
    <div className="mb-12">
      ...
      <div className="relative">
        <div
          ...
          className="flex space-x-6 overflow-x-auto scrollbar-hide scroll-smooth px-2 pb-4"
          style={{ 
            scrollbarWidth: 'none', 
        ...
        >
          {lists.map((list) => (
            <div
              ...
              className="flex-shrink-0 w-80 cursor-pointer group transition-transform duration-200 hover:scale-[1.02]"
            >
              {/* Card Container */}
              <div className="bg-white rounded-2xl border border-gray-200 hover:border-gray-300 transition-all duration-300 hover:shadow-lg cursor-pointer h-full flex flex-col overflow-hidden">
                {/* Image Container */}
                <div className="relative flex-shrink-0">
                  ...
                  {isGuestFavorite(list) && (
                    <div className="absolute top-3 left-3">
                      <div className="bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-semibold text-gray-900 shadow-sm flex items-center space-x-1.5 border border-gray-100">
                  ...
                  <div className="absolute bottom-3 left-3">
                    <div className="bg-black/70 text-white px-2.5 py-1 rounded-full text-xs font-medium backdrop-blur-sm">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center space-x-1.5 flex-shrink-0 bg-gray-50 px-2 py-1 rounded-full">
                      <svg className="w-3.5 h-3.5 text-rose-500" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                      </svg>
